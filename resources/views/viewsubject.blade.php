...
<body>
    ...
    <h2 style="margin-top: 10px">Subject - {{ $subject->subject }}</h2><hr>

    <div class="form-container">
        <h3>Teachers:</h3>
        <div class="mb-3">
            @foreach ($subject->teacher as $list)
                <a href="{{ route('viewteacher', $list->id) }}" class="btn btn-primary"><b>{{ $list->t_name }}</b></a>
            @endforeach
        </div>
    </div>

    <form action="{{ route('viewsubject',$subject->id) }}" method="GET" class="mb-3">
